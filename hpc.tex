...
(5.5,-10) -- (5.5,-8) node [black,xshift=0.8cm] {};
		...
		\caption{First step for odd-numbered process}
		\label{fig:stepone}
		\end{minipage}
	\end{minipage}	
	\hfill
...
(5.5,-10) -- (5.5,-8) node [black,xshift=0.8cm] {};
		...
		\caption{Second step for odd-numbered process}
		\label{fig:steptwo}
		\end{minipage}
\end{figure}
...
The choice of dividing the work along the vertical axis (bigger in size) turned out to be the more beneficial one. As can be seen in in the strong scaling, Figure \ref{fig:speedup}, Increasing the number of cores for a fixed grid size gives diminishing returns, as more time is lost with communications. Also, there is a maximum number of processors that can be used to calculate a simulation of fixed size, as each process needs \textcolor{red}{at least 9 rows to be able to function properly.}\\
