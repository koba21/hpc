...
For a process to be able to calculate a given pixel of its grid, it needs information of its four neighbouring pixels. Thus, to calculate the values of the lower and upper most pixels within its grid part, each process had to have access to the values the process below or above of it had calculated. As illustrated in Figure \ref{fig:tikz}, this could be achieved by letting the different processes communicate to each other in the following way:
	\end{minipage}	
	\hfill
	\begin{minipage}{0.28\linewidth}
		\resizebox{0.9\linewidth}{1.4\linewidth}{
		\begin{tikzpicture}[>=triangle 60]
			\draw (0, 2) -- (0, 0) -- (5, 0) -- (5, 2);
...
As can be seen in Figure \ref{fig:runtimes}, the standard deviation on some of the runtimes can be quite big. This is mainly due that the whole run (increasing from 1 to 36 processes) was only performed 10 times. If one of these times is abnormally high, averaging over only 10 values will not suppress this spike well enough. 
